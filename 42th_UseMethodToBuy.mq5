...
double   EquityNassos=AccountInfoDouble(ACCOUNT_EQUITY);
double   BallanceNassos=AccountInfoDouble(ACCOUNT_BALANCE);
double   PrifitNassos=AccountInfoDouble(ACCOUNT_PROFIT);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Sell_N_request(BidNassos,AskNassos,0.01);
if(PositionsTotal()<10 && Buy_Volume()<10)
Buy_N_request(BidNassos,AskNassos,0.01);
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Comment ("Positions Total: " , PositionsTotal() , "\n"
          "Equity Nassos: "  , EquityNassos,  "\n"
          "BallanceNassos Nassos: "  , BallanceNassos,  "\n"
...
{  TotalBuyVolume=0;// I make it zero to start count from the begining
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(Symbol()==Position_Symbol && PositionType==POSITION_TYPE_BUY)// if currency pair is equal
...
{  TotalSellVolume=0;// I make it zero to start count from the begining
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      double PositionVolume = PositionGetDouble(POSITION_VOLUME);//Get the volume of the position
      double PositionMagic=PositionGetInteger(POSITION_MAGIC);
      if( Symbol()==Position_Symbol && PositionType==POSITION_TYPE_SELL)
...
{TotalBuyVolume=0;// I make it zero to start count from the begining
   ...
   if(N_Position.SelectByIndex(i)) //Tsimpa to index tou position      
      if(N_Position.Symbol()==_Symbol &&N_Position.Type()=="buy")//&& N_Position.Magic()==Magic) // Edo learn to do the above with N_position object
